...
- 新規id(author/detective/translator/publisher/theme/award)は既存と重複していればスキップ
- detective は creatorAuthorId が (既存 + このバッチの新規著者) に存在するか検証し、
  存在しなければその detective 自体を反映せずレポートする
- work は authorIds/detectiveIds/translatorIds/publisherId/themeIds/awardResults[].awardId が
...
def main():
    ...
    detective_ids = {d["id"] for d in detectives}
    ...
    report = {"added": {}, "skipped_duplicates": {}, "rejected_detectives": [], "rejected_works": []}
    ...
    # Authors first: a new detective's creatorAuthorId may point at an author from this same batch.
    ...
    added_detectives, skipped_detectives = [], []
    for d in batch.get("newDetectives", []):
        if d["id"] in detective_ids:
            skipped_detectives.append(d["id"])
            continue
        if d.get("creatorAuthorId") not in author_ids:
            report["rejected_detectives"].append(
                {"id": d["id"], "reason": f"missing creatorAuthorId:{d.get('creatorAuthorId')}"}
            )
            continue
        detectives.append(d)
        detective_ids.add(d["id"])
        added_detectives.append(d["id"])
    report["added"]["detectives"] = added_detectives
    report["skipped_duplicates"]["detectives"] = skipped_detectives
    ...
    for w in batch.get("works", []):
        ...
        for did in w.get("detectiveIds", []):
            if did not in detective_ids:
                missing.append(f"detectiveId:{did}")
