...
const detectives = readSource("detectives");
...
for (const w of works) {
  ...
  w.detectiveIds.forEach((id) => checkRef(detectivesById, id, "detective", w.id));
...
for (const d of detectives) {
  if (!authorsById.has(d.creatorAuthorId)) {
    errors.push(`detective "${d.id}": unknown creatorAuthorId "${d.creatorAuthorId}"`);
  }
  if (d.firstAppearanceWorkId && !workIds.has(d.firstAppearanceWorkId)) {
    errors.push(`detective "${d.id}": unknown firstAppearanceWorkId "${d.firstAppearanceWorkId}"`);
  }
}
...
// Cross-reference lists (author/detective/translator/publisher/theme pages) embed the full
...
// ---- generated/detectives.json ----
// Cases stay in publication order (not the site-wide kana sort), because "what order do I read
// this series in?" is the question a detective page exists to answer.
const worksByDetective = groupWorksBy((w) => w.detectiveIds);
const detectivesGenerated = detectives
  .map((d) => {
    const theirWorks = worksByDetective.get(d.id) ?? [];
    return {
      ...d,
      creatorAuthorName: authorsById.get(d.creatorAuthorId).name,
      workCount: theirWorks.length,
      works: theirWorks.map(fullWork).sort(byPublicationYear),
    };
  })
...
writeFileSync(path.join(outDir, "detectives.json"), JSON.stringify(detectivesGenerated), "utf-8");
...
console.log(
  `generate-manifest: wrote ${works.length} works, ${authors.length} authors, ${detectives.length} detectives, ${translators.length} translators, ${publishers.length} publishers, ${themes.length} themes, ${awards.length} awards`
);
...
const searchIndex = {
  site: "mystery",
  siteName: "ミステリDB",
  baseUrl: "https://izenmi.github.io/mystery-db",
  itemPath: "works",
  items: works.map((w) => ({
    i: w.id,
    t: w.title,
...
// ---- sitemap.xml ----
// Lives at the site root (not data/generated/) so it's served at /mystery-db/sitemap.xml, but is
// just as deterministically derived from public/data/source/*.json — see the .gitignore note.
const SITE_URL = "https://izenmi.github.io/mystery-db";
...
const sitemapEntries = [
  ...
  urlEntry("/detectives"),
  ...detectives.map((d) => urlEntry(`/detectives/${d.id}`, d.updatedAt?.slice(0, 10))),
