...
annot.json(キーを短くしてあるのは手書き量を減らすため):
{
  ...
  "newDetectives":  [{"id":..,"name":..,"kana":..,"desc":..,"creator":"authorId","first":"workId"?,"job":..}],
  ...
  "works": [
    {"n": 3, "id": "kokushikan", "a": ["yokomizo-seishi"], "d": ["kindaichi-kosuke"],
     "tr": ["tanaka-taro"], "p": "kadokawa", "th": ["honkaku","kotou"],
     "o": "jp",              # "jp" | "ov"
     "fy": 1947,             # 原著発表年(省略時は probe の year)
     "jy": 1953,             # 邦訳初刊年(海外作品のみ)
     "ot": "The ...",        # 原題(海外作品は必須)
     "series": "金田一耕助シリーズ", "status": "completed",
...
def main():
    ...
    batch = {"newAuthors": [], "newDetectives": [], "newTranslators": [],
    ...
    for x in annot.get("newDetectives", []):
        d = person(x)
        d["creatorAuthorId"] = x["creator"]
        if x.get("first"):
            d["firstAppearanceWorkId"] = x["first"]
        if x.get("job"):
            d["occupation"] = x["job"]
        batch["newDetectives"].append(d)
    ...
    print(f"works={len(batch['works'])} authors={len(batch['newAuthors'])} "
          f"detectives={len(batch['newDetectives'])} translators={len(batch['newTranslators'])} "
