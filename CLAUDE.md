...
- **楽天の認証情報は姉妹サイトと共用できる**(現行の新gateway形式は各サイトのRefererから通ることを2026-08-04に実証済み)ので新規アプリ登録は不要。`RAKUTEN_APP_ID`/`RAKUTEN_ACCESS_KEY` を渡さずに実行すると BOOK☆WALKER のみで解決する
...
- **楽天ブックス経由の表紙取得が未実行(24作品中16作品=66.7%が解決済み)**: scaffold時点では`RAKUTEN_APP_ID`/`RAKUTEN_ACCESS_KEY`が手元になかったため、BOOK☆WALKERのみで解決している。紙の書籍が中心の古典・文庫作品は楽天ブックスの方が強いので、認証情報を渡して`npm run fetch-covers -- --retry-misses`を実行するとカバー率が上がる見込み。未解決は8作品:
  - 自動で見つからなかった7作品: 本陣殺人事件、獄門島、火車、64、満願、氷菓、Yの悲劇
  - **手動で除去した1作品**: 緋色の研究。BOOK☆WALKERの自動マッチが『【大活字シリーズ】英語原文で味わうSherlock Holmes1』(英語原文の学習用版)を拾ってしまうため、`covers-cache.json`のエントリを`coverUrl: null`に戻して`note`に経緯を記録した。新潮文庫(延原謙訳)の電子版はBOOK☆WALKERの検索では確認できていない(『シャーロック・ホームズの冒険』『同 叡智』はヒットする)。**`--force`で回すとこの誤マッチが復活するので、再挑戦には必ず`--retry-misses`を使うこと**
