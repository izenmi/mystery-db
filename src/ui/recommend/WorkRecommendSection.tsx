 ...
 *
 *  1シードあたりの式はビルド側 relatedIdsFor と同じ: テーマIDFコサイン + 同著者+0.15 + 同探偵+0.1
...
function scoreBySeeds(works: WorkGenerated[], seeds: WorkGenerated[]) {
  ...
  const seedData = seeds.map((s) => ({
    ...
    detectives: new Set(s.detectiveIds),
  }));
  ...
  for (const w of works) {
    ...
    for (const sd of seedData) {
      ...
      if (w.detectiveIds.some((id) => sd.detectives.has(id))) score += SAME_DETECTIVE_BONUS;
      total += score;
    }
        ...
        <>
          ...
          {results.length > 0 && (
            <p className="page-subtitle">
              ...
              同じ著者・探偵の作品には加点しているため、100%を上限に丸めています。
            </p>
          )}
        </>
      )}
