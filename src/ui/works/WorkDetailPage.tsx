          ...
          <div className="work-detail__hero">
            <div className="work-detail__hero-cover">
              ...
              <a
                ...
                href={amazonSearchUrl(state.data.title, state.data.authorNames[0])}
                target="_blank"
                rel="noreferrer"
              >
                Amazonで購入
            ...
            <div className="work-card__body">
              ...
              {state.data.detectiveIds.length > 0 && (
                <p className="page-subtitle">
                  探偵:{" "}
                  {state.data.detectiveIds.map((detectiveId, i) => (
                    <span key={detectiveId}>
                      {i > 0 && "・"}
                      <Link to={`/detectives/${detectiveId}`}>{state.data!.detectiveNames[i]}</Link>
                    </span>
                  ))}
                </p>
              )}
