  ...
  useSeo({
    ...
    description:
      worksState.status === "ready"
        ? `推理小説・ミステリ${worksState.data.length}作品を発表年・五十音・テーマ・探偵・出版社などから検索・絞り込みできます。`
        : undefined,
  });
  ...
  return (
    <div className="page">
      ...
      <div className="filter-row">
        ...
        {detectivesState.status === "ready" && (
          <select value={detectiveId} onChange={(e) => updateParam("detective", e.target.value)}>
            <option value="">探偵で絞り込み</option>
            {detectivesState.data.map((d) => (
