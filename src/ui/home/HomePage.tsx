...
const BADGES: { key: keyof Awaited<ReturnType<typeof getCounts>>; label: string; to: string; color: string }[] = [
  { key: "works", label: "作品", to: "/works", color: "iris" },
  { key: "authors", label: "著者", to: "/authors", color: "pink" },
  { key: "detectives", label: "探偵", to: "/detectives", color: "blue" },
...
export function HomePage() {
  ...
  useSeo({
    description:
      state.status === "ready"
        ? `国内外の推理小説・ミステリ${state.data.works}作品を著者・名探偵・翻訳者・出版社・受賞歴・テーマから検索できるファンデータベース。`
        : undefined,
    jsonLd: {
  ...
  return (
    <div className="page">
      <div className="home-hero">
        <h1 className="font-display">ミステリDB</h1>
        <p className="page-subtitle">国内外の推理小説を著者・名探偵・受賞歴・テーマから探せるデータベース</p>
        <p className="home-intro">
          次に読むミステリ探しに使えるデータベースです。テーマ・探偵・著者・出版社などで絞り込めます。真相に触れる記述は載せていません。
        </p>
      </div>
