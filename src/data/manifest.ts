...
export const getDetectives = () => fetchJson<DetectiveGenerated[]>("detectives.json");
...
export async function getDetective(detectiveId: string): Promise<DetectiveGenerated | undefined> {
  const detectives = await getDetectives();
  return detectives.find((d) => d.id === detectiveId);
}
